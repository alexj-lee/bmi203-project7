...
def one_hot_encode_seqs(seq_arr: List[str]) -> ArrayLike:
    ...
    max_length = len(
        max(seq_arr, key=len)
    )  # find longest sequence so we can specify how long to pad the entries
    ...
    for seq_idx, seq in enumerate(seq_arr):
        ...
        encodings.append(
            np.array(encoding).flatten()
        )  # flatten encoding to 1xN before appending
...
def sample_seqs(seqs: List[str], labels: List[bool]) -> Tuple[List[str], List[bool]]:
    ...
    We will oversample the undersampled class and keep in all the negative examples so we don't "lose" any data.
    ...
    uniq_labels = collections.defaultdict(
        list
    )  # we will append to these lists las we loop through (seq, labels)

    for idx, (seq, label) in enumerate(zip(seqs, labels)):
        if isinstance(label, int) is False:
            raise TypeError(
                "Labels must be of type int"
            )  # in case i passsed a float or some other wrong thingaccidentally

        uniq_labels[label].append(idx)

        if len(uniq_labels.keys()) > 2:
            raise ValueError(
                "Must be only two classes in dataset"
            )  # lets just assume binary clf

    undersampled_class = min(
        uniq_labels, key=uniq_labels.get
    )  # will be the class label int of the undersampled class
    oversampled_class = [
        key for key in uniq_labels.keys() if key != undersampled_class
    ][0]

    ratio = len(uniq_labels[oversampled_class]) // len(
        uniq_labels[undersampled_class]
    )  # how much to oversample by

    undersampled_indices = uniq_labels[undersampled_class] * ratio

    all_data = []  # will be a list of (x, y) pairs
    ...
    sampled_seqs, sampled_labels = list(
        zip(*all_data)
    )  # will be two lists, x list and y list
